...
export default function UserTrack() {
    ...
    useEffect(() => {
        ...
        setToken(user.token);
        if (user.token) fetchMyTickets(user.token);
    }, []);
                        ...
                        {tickets.length > 0 ? tickets.map(tk => {
                            ...
                            return (
                                <tr key={tk.id}>
                                    ...
                                    <td title={tk.description}>{tk.description?.substring(0, 40)}...</td>
                                    <td>
                ...
                {tickets.length === 0 ? (
                    ...
                    return (
                        <div key={tk.id} style={{
                            ...
                            {/* Row 5: Detail Button */}
                            <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: '0.25rem', borderTop: '1px solid var(--color-glass-border)' }}>
                                <button
